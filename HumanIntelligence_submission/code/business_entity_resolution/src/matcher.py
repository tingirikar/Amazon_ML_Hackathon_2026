...
FEATURE_NAMES = [
    ...
    'addr_digit_full_match',         # 9: 1.0 if all digits match exactly (NEW)
    # --- Token-level features (10-11) ---
    'first_token_match',             # 10: First root word matches
    'name_word_jaccard',             # 11: Word-level Jaccard on cleaned names (NEW)
    ...
    'addr_word_jaccard',             # 19: Word-level Jaccard on addresses (NEW)
...
def extract_pairwise_features(
    ...
    Parameters
    ----------
    s1_name, s1_core, s1_addr : str
        ASCII-cleaned Source 1 fields.
    s1_digits : set
        Digit tokens from Source 1 address.
    c_name, c_core, c_addr : str
        ASCII-cleaned candidate fields.
    c_digits : set
        Digit tokens from candidate address.
    s1_uname, c_uname : str
        Unicode-preserved (clean_unicode) business names.
    s1_uaddr, c_uaddr : str
        Unicode-preserved (clean_unicode) addresses.
    s1_non_ascii, c_non_ascii : bool
        Whether the original name contained non-ASCII characters.
    """
    # --- Digit features ---
...
class EntityMatcher:
    ...
    def __init__(self, model_path="models/lgbm_matcher.txt", threshold=0.50):
